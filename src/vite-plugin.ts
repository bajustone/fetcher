...
function emitEnvDeclaration(
  ...
  // IMPORTANT — resolution of `./paths` inside ambient `declare module`:
  //
  // TypeScript is unreliable at resolving `import type { X } from './relative'`
  // statements placed inside a `declare module '...'` block when the
  // containing file has no top-level imports/exports (i.e. is a script).
  // The imported `X` often collapses to an opaque shape in consumers —
  // observable downstream as `PathsToRoutes<paths>` evaluating to
  // `{ [k: string]: { [m]: { body?: Schema<never>, ... } } }` (keys are
  // `string` instead of the literal path union; every slot's `T` is `never`).
  //
  // We cannot simply hoist the imports to the top level: that would make
  // the file a module, and `declare module 'X'` inside a module file is a
  // *module augmentation*, not a new ambient module declaration — so
  // consumers would get "Cannot find module 'virtual:fetcher'".
  //
  // The robust fix is the dynamic-import type syntax:
  //   `import('./paths').paths`
  // which resolves relative paths reliably in any context (ambient
  // declare-module blocks inside scripts included) and keeps the file a
  // script so the ambient module declaration remains a declaration.
  const pathsRef = `import('./paths').paths`;
  const componentsRef = `import('./paths').components`;

  // Narrow validator outputs to the component's declared type — only when
  // we have component names to key on. Otherwise fall back to unknown.
  // Uses the same dynamic-import type syntax as `pathsRef` for the same
  // script-context resolution reasons documented above.
  const validatorsBody = componentNames
    .map(n => hasTypedComponents
      ? `    ${JSON.stringify(n)}: StandardSchemaV1<unknown, ${componentsRef}['schemas'][${JSON.stringify(n)}]>;`
  ...
  const lines: string[] = [
    ...
    `   * to \`Schema<ConcreteT>\` instead of the bare \`Schema\`. Method keys`,
    `   * are uppercase, matching the runtime table populated by the plugin.`,
    `   */`,
    `  export const routes: PathsToRoutes<${pathsRef}>;`,
  ];
  ...
  if (emitComponents) {
    lines.push(
      ...
      `   * — \`validators.Pet\` is \`StandardSchemaV1<unknown, import('./paths').components['schemas']['Pet']>\`.`,
      `   */`,
      validatorsBody
